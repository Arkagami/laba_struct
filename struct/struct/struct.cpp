...
void dels() {
	...
	char s[1000];
	printf("Ââåäèòå ID ýëåìåíòà:");
	s[0]='\0';
	gets_s(s, 999);
	int id = StrToInt(s);
	while ((intLi(s) == 0) || (id < 0) || (id > ID)) {
		printf("Âû ââåëè íåâåðíîå ÷èñëî. Ïîâòîðèòå ââîä:");
		s[0]='\0';
		gets_s(s, 999);
		id = StrToInt(s);
	}
...
void ins() {
	...
	printf("Íàçâàíèå âîîðóæåíèÿ (áîëåå 19 áóêâ îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);	
	s[19] = '\0';
	strcpy(one->name, s);

	printf("Íîìåð (áîëåå 5 öèôð îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	int y = StrToInt(s);
	while (intLi(s) == 0) {
	...
	gets_s(s, 999);
	myStrcmp(s, "Ñèíèé");
	while (!myStrcmp(s, "Ñèíèé") && !myStrcmp(s, "ñèíèé") && !myStrcmp(s, "Êðàñíûé") && !myStrcmp(s, "êðàñíûé") && !myStrcmp(s, "Çåëåíûé") && !myStrcmp(s, "çåëåíûé") && !myStrcmp(s, "Æåëòûé") && !myStrcmp(s, "æåëòûé")) {
		printf("Âû ââåëè íåâåðíûé öâåò. Ïîâòîðèòå ââîä:");
		s[0]='\0';
		gets_s(s, 999);
	}
	...
	printf("Óðîí (áîëåå 4 öèôð îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	y = StrToInt(s);
	while ((intLi(s) == 0) || (y == -1)) {
	...
	printf("Çäîðîâüå (áîëåå 4 öèôð îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	y = StrToInt(s);
	while ((intLi(s) == 0) || (y == -1)) {
	...
	printf("Âðåìÿ ðàáîòû (äðîáüþ ÷åðåç òî÷êó íå áîëåå 8 ñèìâîëîâ):");
	s[0]='\0';
	gets_s(s, 999);
	y = StrToInt(s);
	while ((intLi(s) == 0) || (y == -1)) {
	...
	printf("Ñòîèìîñòü â ðóáëÿõ (áîëåå 9 öèôð îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	y = StrToInt(s);
	while ((intLi(s) == 0) || (y == -1)) {
	...
	printf("Øîê â ìñ (áîëåå 4 öèôð îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	y = StrToInt(s);
	while ((intLi(s) == 0) || (y == -1)) {
	...
	y = StrToInt(s);
	one->shock = y % 10000;
	printf("Èìÿ âëàäåëüöà (áîëåå 13 áóêâ îáðåçàåòñÿ):");
	s[0]='\0';
	gets_s(s, 999);
	s[13] = '\0';
...
void finds() {
	...
	char s[1000];
		...
		u = -20;
		printf("0 - ID\n");
		printf("1 - Íàçâàíèå âîîðóæåíèÿ\n");
		printf("2 - Íîìåð\n");
		printf("3 - Öâåò\n");
		printf("4 - Óðîí\n");
		printf("5 - Çäîðîâüå\n");
		printf("6 - Âðåìÿ ðàáîòû\n");
		printf("7 - Ñòîèìîñòü\n");
		printf("8 - Øîê\n");
		printf("9 - Èìÿ âëàäåëüöà\n");
		gets_s(s, 999);
		u = StrToInt(s);
		while ((intLi(s) == 0) || (u == -1)) {
		repeats:;
			printf("Âû ââåëè íåâåðíîå ÷èñëî èëè ñòðîêó. Ïîâòîðèòå ââîä:");
			gets_s(s, 999);
			u = StrToInt(s);
		}
		system("cls");
...
int main()
{
	...
	while (1) {
		...
		printf("0 - Âûâåñòè áàçó äàííûõ íà ýêðàí\n");
		printf("1 - Äîáàâèòü ýëåìåíò\n");
		printf("2 - Íàéòè ýëåìåíò\n");
		printf("3 - Óäàëèòü ýëåìåíò\n");
		printf("4 - Óäàëèòü âñþ áàçó\n");
		gets_s(s, 999);
		u = StrToInt(s);
		while ((intLi(s) == 0) || (u == -1)) {
		repeat:;
